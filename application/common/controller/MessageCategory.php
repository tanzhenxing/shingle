...
class MessageCategory extends Controller
{
    /**
     * 推送数据
     * @param $data
    ...
    public static function push($data)
    {
        if (!isset($data['title']) or empty($data['title'])) {
            $result = array('code'=>1,'message'=>'title不能为空');
            return $result;
        }
        if (!isset($data['unique_code']) or empty($data['unique_code'])) {
            $result = array('code'=>1,'message'=>'unique_code不能为空');
            return $result;
        }
        $get_data = \app\common\model\MessageCategory::get(['extension'=>$data['extension']]);
        if (empty($get_data)) {
    ...
    public static function create($data)
    {
        ...
        // 验证数据内容是否合法
        $validate = new \app\common\validate\MessageCategory();
        if (!$validate->check($data)) {
            $result = array('code'=>1,'message'=>$validate->getError());
            return $result;
        }
        // 保存数据
        $obj = new \app\common\model\MessageCategory();
        ...
        // 重新获取最新的数据
        $query = array('unique_code'=>$data['unique_code']);
