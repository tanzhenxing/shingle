...
class MessageRead extends Controller
{
    /**
     * 保存数据记录
     * @param $data
    ...
    {
        // 获取数据
        $query = array('message_id'=>$data['message_id'],'user_id'=>$data['user_id']);
        $obj = new \app\common\model\MessageRead();
        $get_data = $obj->get($query);
        if (empty($get_data)) { // 新建记录
            $result = static::create($data);
        } else { // 更新记录
            unset($data['message_id'],$data['user_id']);
            $data['id'] = $get_data['id'];
    ...
    public static function create($data)
    {
        ...
        $validate = new \app\common\validate\MessageRead();
