...
var replaceDevCode = function (list) {
    list.forEach(function (fileName) {
        ...
        newContent = newContent.replace(/https:\/\/\w+\.com\/[\w\-]+\/server\//, 'https://chat.com/');
        newContent = newContent.replace(/test-125\d{7}/, 'hk-1251279962');
        newContent = newContent.replace(/'proxy' => 'http:\/\/[^']+',/, "'proxy' => '',");
        newContent = newContent.replace(/proxy: 'http:\/\/[^']+',/, "proxy: '',");
        newContent = newContent.replace(/AKID\w+/, 'AKIDU9lDeB2GG7reLsAeq9GI9rQ2ndsKE7yH');
        newContent = newContent.replace(/'secretKey' => '[^']+',/, "'secretKey' => 'fQYgw59X24zzxUWcgs4EdF8ixEUU2yuo',");
        newContent = newContent.replace(/secretKey: '[^']+',/, "secretKey: 'fQYgw59X24zzxUWcgs4EdF8ixEUU2yuo',");
